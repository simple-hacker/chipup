<template>
	<div>
		<h1 class="text-lg md:text-2xl text-gray-700">Start a new cash game session.</h1>
    	<hr class="mb-4">
    	<form action="" @submit.prevent="startCashGame">
			<div class="flex flex-col">
				<div>
					<p class="text-base md:text-lg text-gray-700 mb-2">What are you playing?</p>
					<div class="flex flex-wrap">
						<div class="w-1/2 lg:w-1/4 p-1">
							<select v-model="stake" class="w-full bg-white border p-2 mb-2" :class="errors.stake_id ? 'border-red-700' : 'border-gray-400'">
						...
						<div class="w-1/2 lg:w-1/4 p-1">
							<select v-model="limit" class="w-full bg-white border p-2 mb-2" :class="errors.limit_id ? 'border-red-700' : 'border-gray-400'">
						...
						<div class="w-1/2 lg:w-1/4 p-1">
							<select v-model="variant" class="w-full bg-white border p-2 mb-2" :class="errors.variant_id ? 'border-red-700' : 'border-gray-400'">
						...
						<div class="w-1/2 lg:w-1/4 p-1">
							<select v-model="table_size" class="w-full bg-white border p-2 mb-2" :class="errors.table_size_id ? 'border-red-700' : 'border-gray-400'">
				...
				<div class="mt-3">
					<p class="text-base md:text-lg text-gray-700 mb-2">Where are you playing?</p>
					<input v-model="location" type="text" placeholder="Enter location" class="w-full border p-3" :class="errors.location ? 'border-red-700' : 'border-gray-400 mb-2'"/>
					<span v-if="errors.location" class="text-xs text-red-700">{{ errors.location[0] }}</span>
				</div>
				<div class="mt-3">
					<p class="text-base md:text-lg text-gray-700 mb-2">What's your buyin?</p>
					<input v-model="buyin" type="number" step="0.01" min="0" class="w-full border p-3" :class="errors.buyin ? 'border-red-700' : 'border-gray-400 mb-2'"/>
				...
				<transition name="fade-slide" mode="out-in">
					...
					<div v-else key="notLive">
						<div class="mt-4">
							<p class="text-base md:text-lg text-gray-700 mb-2">When did this cash game start?</p>
							<datetime
								v-model="start_time"
								type="datetime"
								...
								class="w-full border theme-green"
						...
						<div class="mt-4">
							<p class="text-base md:text-lg text-gray-700 mb-2">When did this cash game end?</p>
							<datetime
								v-model="end_time"
								type="datetime"
								input-class="w-full p-3"
						...
						<div class="mt-4">
							<button type="button" class="w-full bg-green-700 border border-green-800 hover:bg-green-800 p-4 uppercase text-white font-bold text-center">Save cash game</button>
						</div>
						<div class="mt-8 flex justify-center">
							<a @click.prevent="live = true" href="#" class="text-green-700 hover:text-green-800 text-sm font-bold">Switch to live mode</a>
						</div>
					</div>
				</transition>
			</div>
