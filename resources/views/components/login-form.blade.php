<form action="/login" method="POST" class="w-full flex flex-col mb-5">
    ...
    <div class="flex flex-col md:flex-row justify-around items-center mt-3">
        <div class="flex items-center">
            <input type="checkbox" name="remember" class="w-auto mr-2">
            <label for="remember" class="w-full text-sm uppercase tracking-wider font-bold text-gray-200">{{ __('Remember me') }}</label>
        </div>
        <a href="/password/reset" class="text-green-500 hover:text-green-400 uppercase text-sm font-bold tracking-wide mt-4 md:mt-0">{{ __('Forgot your password?') }}</a>
    </div>
</form>
